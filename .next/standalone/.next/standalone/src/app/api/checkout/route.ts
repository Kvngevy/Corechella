...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (result.order.total < WAVY_MIN_AMOUNT_NGN) {
      const completed = await withDb((state) => {
        const confirm = confirmOrderFromWebhook(state, result.order.id);
        if (!confirm.success) return null;
        const attendees = ensureOrderAttendees(state, confirm.order);
        return { order: confirm.order, attendees };
      });

      if (!completed) {
        await withDb((state) => cancelPendingOrder(state, result.order.id));
        return NextResponse.json({ error: "Could not finalize your order" }, { status: 500 });
      }

      return NextResponse.json({
        orderId: result.order.id,
        order: completed.order,
        attendees: completed.attendees,
        guestAccess,
      });
    }
    ...
    if (pendingOrderId) {
      try {
        await withDb((state) => cancelPendingOrder(state, pendingOrderId!));
      } catch (rollbackErr) {
        logger.error("Failed to rollback pending order after checkout error", {
    ...
    logger.error("Checkout failed", { error: String(err), orderId: pendingOrderId });
